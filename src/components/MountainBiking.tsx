...
function drawMountain(layer: Konva.Layer, stageWidth: number, ground: number): void {
    const elevations = [
        ground,
        ground - 20,
        ground -  20,
    ...
    for (let i = 0; i < elevations.length; i++) {
        points.push(
            stageWidth * (i / elevations.length), 
            elevations[i] 
        )
    }
...
function drawGrid(layer: Konva.Layer, stageWidth: number, stageHeight: number): void {
    const gridFactor = 10
...
function createStage(container: HTMLDivElement | null): Konva.Stage | undefined {
    if (container ) {
...
export default function MountainBiking({ className }: { className?: string }) {
    return (
        <Stage ref={createStage} className={className}></Stage>
    )
}
